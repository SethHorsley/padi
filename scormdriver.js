    ...
    voidStatement: function (stmt, callback, options) {
      ...
      if (rsCount > 0) {
        ...
        for (i = 0; i < rsCount; i += 1) {
          lrs = this.recordStores[i];
          results.push(
            lrs.saveStatement(voidingStatement, {
              callback: callbackWrapper,
            }),
          );
        }
...
(function (root, factory) {
  ...
  URI.find_uri_expression =
    /\b((?:[a-z][\w-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?Â«Â»]))/gi;
  URI.findUri = {
...
function CMI5_GetPreviouslyAccumulatedTime() {
  var retrievedValue;
  WriteToDebug(
    "In CMI5_GetPreviouslyAccumulatedTime - cached: " +
      CMI5_TOTAL_PREV_DURATION,
  );
...
function LMSStandardAPI(strStandard) {
  ...
  eval(
    "this.CreateValidIdentifier = " + strStandard + "_CreateValidIdentifier",
  );
